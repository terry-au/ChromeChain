...
- (void)trySaveUsername:(NSString *)username password:(NSString *)password realm:(NSURL *)realm {
    ...
    [alert runModal];
//            [KeychainWriter writeCredentialToKeychain:loginCredential decryptedPassword:decryptedPasswordString];
//    NSURLCredential *urlCredential = [NSURLCredential credentialWithUser:username password:password persistence:NSURLCredentialPersistenceSynchronizable];
    NSURLCredential *urlCredential = [[NSURLCredential alloc] safari_initWithUser:username password:password persistence:NSURLCredentialPersistenceNone];
//    NSURLProtectionSpace *protectionSpace = [[NSURLProtectionSpace alloc] initWithHost:realm port:80 protocol:nil realm:realm authenticationMethod:@"form"];
    ...
    NSLog(@"safari_addSynchronizableCopyOfCredentialIfNeeded: %@", [sharedCredentialStorage safari_addSynchronizableCopyOfCredentialIfNeeded:urlCredential forProtectionSpace:protectionSpace]);
//    [sharedCredentialStorage credentialsForProtectionSpace:<#(NSURLProtectionSpace *)space#>]
...
- (void)tableViewDoubleAction:(id)sender {
    ...
    DecryptionResult decryptionResult = [_passwordDecryptor decryptPassword:loginCredential.encryptedPasswordString decryptedPassword:&decryptedPassword];
